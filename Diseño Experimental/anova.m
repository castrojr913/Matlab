...
%
%NOTA: cada fila debe tener igual tamaño, lo mismo cada columna.
%si es desbalanceado, colocar NaN a aquéllas unidades que no estén. 
%El NaN le dice al programa que esa unidad está ausente en la tabla.
%gráficas: escribir sí->'s' no->cualquier letra excepto s
clear 
close all 
cTt=0;cnan=0;
...
graph=input('Gráfica de la distribución F (s/n): ','s');
if graph=='s'
...
for j=1:s2
    if cTt~=0 %¿realizó conteo de unidades por tto? 
       ...
       Tt5(j-1)=sqrt(Tt4(j-1));%desv.éstandar (hasta j-1 tto)
    end                          
...
end %N: total unidades del diseño
N=s1*s2-cnan;Tt3(j)=(Tt(j))^2/cTt;%suma total al cuadrado para tto j 
mTt(j)=Tt(j)/cTt;fc=(sum(Tt))^2/N;%factor de corrección
...
disp(Tt4);disp(' Desviación Estándar/tto:')
disp(Tt5);disp(' ')
%-------ANOVA--------
disp('Fuente de variación   Suma de cuadrados  Grados de libertad  Cuadrados medios       F       Valor crítico F') 
...
if graph=='s' %fpdf->f. densidad de prob. F
    ...
    set(gr1,'linewidth',2);grid on %aumentar grosor de la línea 
    title('Distribución F con F tabulada')
    xlabel('F');ylabel('f(F)');hold on %labels y sostener gráfica
    plot(ones(1,length(ff))*ft,ff,'--r')%indicar Ft y área en f(F) al alpha 
end
%----Hipotesis----
disp(' ')
if fc>ft
   fprintf('Conclusión: rechazar hipótesis nula pues F=%5.4f>%5.4f\n',fc,ft) 
else
   fprintf('Conclusión: no rechazar hipótesis nula pues F=%5.4f<=%5.4f\n',fc,ft) 
end
%---- opcional: calcular la media y varianza de la distribución F ----
%[m,v]=fstat(gl(1),gl(2))
